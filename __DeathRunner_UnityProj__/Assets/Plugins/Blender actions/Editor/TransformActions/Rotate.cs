...
namespace BlenderActions
{
	public class Rotate : BaseTransform
	{
			...
			else
			{
				...
				else
				{
						...
						|| NumericInput.EnteredNumber != 0)// #Color(lime*3);
					{
						...
						CalculateRotation(originalMousePos, BA.CurrentEvent.mousePosition * EditorGUIUtility.pixelsPerPoint);
					}
		...
		{//#colreg(darkcyan);
			...
			if (BA.Use3DCursor)                     //#color(orange*3);
				pivot = BA.The3DCursorPos;          //#color(orange*3);
			Vector2 inSP = sceneCam.WorldToScreenPoint(pivot) * EditorGUIUtility.pixelsPerPoint;
			...
			{
				inSP.y = sceneCam.pixelHeight * EditorGUIUtility.pixelsPerPoint - inSP.y;
				...
				|| NumericInput.EnteredNumber != 0)      // #Color(lime*3);
			{
				...
				PerformNumericSnap();      // #Color(lime*3);

				// Handle vertex snapping ONLY AFTER the original transformations.	#colreg(red*0.3);
				if (!BA.NumericSnap       // #Color(lime*3);
					&& BA.VertexSnapON_ThisFrame)
				{
