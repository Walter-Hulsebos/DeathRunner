    ...
    public class BlenderActions
    {
		...
		public void CheckAndRunHotkeys()
		{
				...
				&& CurrentEvent != null)
			{
				...
				mousePos.y = sceneCam.pixelHeight * EditorGUIUtility.pixelsPerPoint - mousePos.y;
		...
		private void OnSceneGUI(SceneView sceneView)
		{
			...
			{
				Vector2 leftTopGUISpace = new Vector2(sceneCam.pixelRect.xMin + 2, sceneCam.pixelRect.yMin * EditorGUIUtility.pixelsPerPoint + 3);
				Vector2 rightBottomGUISpace = new Vector2(sceneCam.pixelWidth - 2, sceneCam.pixelHeight * EditorGUIUtility.pixelsPerPoint);
				Vector2 leftBottomGUISpace = new Vector2(sceneCam.pixelRect.xMin + 2, sceneCam.pixelHeight * EditorGUIUtility.pixelsPerPoint);
				Vector2 rightTopGUISpace = new Vector2(sceneCam.pixelWidth - 2, sceneCam.pixelRect.yMin * EditorGUIUtility.pixelsPerPoint + 3);
			...
			if (Selection.activeGameObject != null)
			{
				...
				if (topLevelSelectedTransforms != null)
				{
					...
					if (n > 0)
					{
						...
						// First - order GOs by their SiblingIndex.
						List<Transform> sortedTransforms = new List<Transform>(n);
						for (int i = 0; i < n; i++)
							sortedTransforms.Add(topLevelSelectedTransforms[i]);

						var sortedArray = sortedTransforms.Select(x => x).OrderBy(y => y.GetSiblingIndex()).ToArray();

						for (int i = 0; i < sortedArray.Length; i++)
							Undo.SetTransformParent(sortedArray[i], result.transform, "Parent selected to a new empty GameObject");
