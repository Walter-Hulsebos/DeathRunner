...
namespace DeathRunner
{
    public class CardSelectionHandler : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler, ISelectHandler, IDeselectHandler
    {
        ...
        public void OnSelect(BaseEventData eventData)
        {
            ...
            //Find the index
            for (int i = 0; i < CardSelectionManager.instance.Buttons.Length; i++)
            {
                if (CardSelectionManager.instance.Buttons[i] == gameObject)
                {
                    CardSelectionManager.instance.LastSelectedIndex = 1;
                    return;
                }
            }
        }
