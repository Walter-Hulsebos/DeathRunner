...
namespace DungeonArchitect.Editors.LaunchPad
{
    ...
    {
        private static T LoadAsset<T>(string basePath, string path) where T : Object
        {
            var fullPath = "LaunchPad/" + basePath + path;
            return Resources.Load<T>(fullPath);
        }

        private static TextAsset LoadPageAsset(string path)
        {
        ...
        private static ScreenPageType GetPageType(string json)
        {
