...
namespace DungeonArchitect.MarkerGenerator.VM
{
    public class ConditionGraphCompiler
    {
        ...
        private void AssembleProgram(MarkerGenRuleProgram program)
        {
            program.instructions = instructions.ToArray();
            program.stringTable = stringTable.ToArray();
        }

        private void CreateDefaultFalseProgram()
        {
            // Push false and halt
            instructions.Clear();
        ...
        private void AddInstruction(int opcode, int arg0 = 0, int arg1 = 0)
        {
            instructions.Add(new Instruction(opcode, arg0, arg1));
        }

        private int RegisterString(string value)
        {
            stringTable.Add(value);
            return stringTable.Count - 1;
        }

        private Dictionary<GraphPin, GraphPin> BuildPinLookup(Graph graph)
        {
