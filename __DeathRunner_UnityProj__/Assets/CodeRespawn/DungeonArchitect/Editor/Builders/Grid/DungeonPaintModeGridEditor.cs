...
namespace DungeonArchitect.Editors
{
    ...
    public class DungeonPaintModeGridEditor : DungeonPaintModeEditor
    {
        ...
        private void UpdateCursorPosition(DungeonPaintModeGrid mode) {
			float distance;
			var e = Event.current;
			Plane plane;
			if (mode.mode2D) {
        ...
        private void ProcessPaintRequest(Event e)
        {
            var mode = target as DungeonPaintModeGrid;
            if (mode == null) return;
        ...
        private void DrawCursor(GridDungeonConfig config, bool mode2D, int size)
        {
        ...
        private void DrawRect(Vector3 position, Vector3 size, Color color, float faceOpacity, float outlineOpacity, bool mode2D)
        {
        ...
        private void DrawOverlayData(GridDungeonConfig config, GridDungeonModel model, bool mode2D)
        {
            var mode = target as DungeonPaintModeGrid;
