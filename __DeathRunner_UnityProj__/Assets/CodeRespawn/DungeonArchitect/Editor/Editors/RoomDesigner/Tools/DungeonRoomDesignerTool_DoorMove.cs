...
namespace DungeonArchitect.RoomDesigner.Editors
{
    ...
    {
        private float SnapToGrid(float value, float gridValue)
        {
            return Mathf.RoundToInt(value / gridValue) * gridValue;
        }

        private void ClampToWalls(DungeonRoomDesigner room, Vector3 p, out Quaternion rotation, out Vector3 freeMovePosition, out Vector3 snappedPosition)
        {
        ...
        private void DrawDoorEditor(DungeonRoomDesigner room, ref DungeonRoomDoorDesigner door)
        {
