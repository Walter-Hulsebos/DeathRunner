...
namespace DungeonArchitect.Editors.Flow
{
    public abstract class FlowExecNodeRendererBase : GraphNodeRenderer
    {
        ...
        private void DrawPins(UIRenderer renderer, GraphRendererContext rendererContext, GraphNode node, GraphCamera camera, Color nodeColor)
        {
        ...
        private class GridFlowExecEditorConstants
        {
        ...
        private static Color GetPinColor(GraphPin pin, Color nodeColor)
        {
            Color color;
        ...
        private void DrawPin(UIRenderer renderer, GraphRendererContext rendererContext, GraphPin pin, GraphCamera camera, Color nodeColor)
        {
            if (pin.Node == null)
            {
        ...
        private struct NodeBoundLayoutData
        {
            public Vector2 screenTextSize;
            public Vector2 screenNodeSize;
        }

        private static void UpdateNodeBounds(GraphNode node, float zoomLevel, Vector2 contentScreenSize, GUIStyle style, out NodeBoundLayoutData outLayoutData)
        {
        ...
        private void DrawNodeExecStatus(UIRenderer renderer, GraphNode node, GraphCamera camera, Rect boxBounds, ExecRuleNodeLayoutInfo layout)
        {
        ...
        private GUIStyle GetDescStyle(GUIStyle style)
        {
        ...
        private GUIStyle GetErrorStyle(GUIStyle style)
        {
        ...
        private ExecRuleNodeLayoutInfo CalcLayoutInfo(GraphNode node, GUIStyle style)
        {
        ...
        private void DrawNodeBody(UIRenderer renderer, NodeBoundLayoutData layoutData, GraphNode node, GraphCamera camera)
        {
            var style = GetNodeStyle(node, camera.ZoomLevel);
