...
namespace DungeonArchitect.Samples.ShooterGame
{
	public class PatrolPath : MonoBehaviour {
		...
		private Waypoint[] patrolPoints = new Waypoint[0];
		...
		private Waypoint FindNearest(Waypoint[] waypoints, Vector3 startingPoint) {
		...
		private void DrawLine(Vector3 a, Vector3 b, bool mode2D) {
			if (mode2D) {
		...
		private void DrawPoint(Vector3 p, bool mode2D) {
			if (mode2D) {
