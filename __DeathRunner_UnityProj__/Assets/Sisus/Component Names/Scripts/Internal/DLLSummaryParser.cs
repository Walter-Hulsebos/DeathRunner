...
namespace Sisus
{
	...
	internal static class DLLSummaryParser
    {
		...
		{
			if(cachedTooltips.TryGetValue(classType, out summary))
			{
			...
			foreach(object member in members)
			{
				...
				stringBuilder.Clear();
				cachedTooltips[classType] = summary;
				return summary.Length > 0;
			}
		...
		private static string GetXMLDocumentationFilepath(Assembly assembly)
		{
			...
			for(directory = Path.GetDirectoryName(xmlFilePath); directory != null; directory = Path.GetDirectoryName(directory))
			{
				...
				Debug.Log("xmlFileName: " + xmlFileName);
				#endif
