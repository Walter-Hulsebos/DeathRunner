...
namespace Asset_Cleaner {
	internal static class SearchUtils {
		...
		private static Queue<SerializedProperty> _tmp = new Queue<SerializedProperty>();
		...
		private static bool SearchInChildProperties(SearchArg arg, Object suspect, bool scene, out EcsEntity entity) {
		...
		private static bool AnyDependencies(string path) {
			var store = Globals<BacklinkStore>.Value;
		...
		private static Pool<List<Component>> ComponentListPool = new Pool<List<Component>>(() => new List<Component>(), list => list.Clear());
		...
		private static bool IsTargetOrNested(SearchArg target, Object suspect) {
			if (!suspect)
				return false;
		...
		private static string Nicify(SerializedProperty sp, Object o, string transformPath) {
