...
namespace ProjectDawn.Geometry2D
{
    ...
    public static partial class ShapeGizmos
    {
        ...
        public static void DrawLineArrow(Line line, float size, Color color)
        {
            ...
            float arrowAngle = radians(45);
            DrawLine(line.To, line.To + direction(lineAngle + arrowAngle) * size, color);
            DrawLine(line.To, line.To + direction(lineAngle - arrowAngle) * size, color);
        }
        ...
        public static void DrawWireCapsule(Capsule capsule, Color color)
        {
            ...
            DrawLine(from + tangent * capsule.Radius, to + tangent * capsule.Radius, color);
            DrawLine(from - tangent * capsule.Radius, to - tangent * capsule.Radius, color);
#if UNITY_EDITOR
            UnityEditor.Handles.color = color;
            UnityEditor.Handles.DrawWireArc(from.asvector3(), normal, -tangent.asvector3(), 180, capsule.Radius);
            UnityEditor.Handles.DrawWireArc(to.asvector3(), normal, tangent.asvector3(), 180, capsule.Radius);
#endif
        }
