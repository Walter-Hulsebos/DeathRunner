...
namespace ProjectDawn.Geometry2D.LowLevel.Unsafe
{
    ...
    public unsafe struct UnsafeVoronoiBuilder : IDisposable
    {
        ...
        public UnsafeVoronoiBuilder(int initialCapacity, AllocatorManager.AllocatorHandle allocator)
        {
            ...
            m_SiteEvents = new UnsafeLinkedPriorityQueue<SiteEvent, SiteComparer>(initialCapacity, allocator);
            m_IntersectionEvents = new UnsafeLinkedPriorityQueue<IntersectionEvent, IntersectionComparer>((initialCapacity << 2) + (initialCapacity << 1), allocator);
        }
        ...
        public static UnsafeVoronoiBuilder* Create(int initialCapacity, AllocatorManager.AllocatorHandle allocator)
        {
            ...
            data->m_SiteEvents = new UnsafeLinkedPriorityQueue<SiteEvent, SiteComparer>(initialCapacity, allocator);
            data->m_IntersectionEvents = new UnsafeLinkedPriorityQueue<IntersectionEvent, IntersectionComparer>((initialCapacity << 2) + (initialCapacity << 1), allocator);
        ...
        private void FortunesAlgorithm<T>(ref T output) where T : IVoronoiOutput
        {
            ...
            while (true)
            {
                ...
                else if (!m_IntersectionEvents.IsEmpty)
                {
                    ...
                    Assert.AreEqual(RightSite(lbnd), LeftSite(rbnd));

                    Vertex* eventVertex = e.Vertex; // get the vertex that caused this event
                    eventVertex->Index = output.ProcessVertex(eventVertex->Point, bot.SiteIndex, top.SiteIndex, RightSite(lbnd).SiteIndex);
        ...
        [DebuggerDisplay("{SiteIndex} {Point}")]
