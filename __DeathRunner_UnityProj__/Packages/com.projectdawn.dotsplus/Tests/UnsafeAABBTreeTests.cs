...
namespace ProjectDawn.Collections.Tests
{
    ...
    internal class UnsafeAABBTreeTests
    {
        ...
        [Test]
        public void UnsafeAABBTreeTests_Rectangle_CountLeafs()
        {
            var tree = new UnsafeAABBTree<AABRectangle>(1, Allocator.Temp);
            ...
            Assert.AreEqual(4, tree.CountLeafs());

            tree.Dispose();
        }

        [Test]
        public void UnsafeAABBTreeTests_Rectangle_GetDepth()
        {
            var tree = new UnsafeAABBTree<AABRectangle>(1, Allocator.Temp);
        ...
        [Test]
        public void UnsafeAABBTreeTests_Rectangle_GetBalancedTreeFactor_One()
        {
            var tree = new UnsafeAABBTree<AABRectangle>(1, Allocator.Temp);
            ...
            Assert.AreEqual(1, tree.GetBalancedTreeFactor());

            tree.Dispose();
        }

        [Test]
        public void UnsafeAABBTreeTests_Rectangle_GetBalancedTreeFactor_Zero()
        {
            var tree = new UnsafeAABBTree<AABRectangle>(1, Allocator.Temp);

            var a = tree.Add(new AABRectangle(new Rectangle(new float2(5, 5), new float2(1, 1))));
            var b = tree.Add(new AABRectangle(new Rectangle(new float2(6, 5), new float2(1, 1))));

            var c = tree.Add(new AABRectangle(new Rectangle(new float2(7, 5), new float2(1, 1))));
            var d = tree.Add(new AABRectangle(new Rectangle(new float2(10, 5), new float2(1, 1))));

            Assert.AreEqual(0, tree.GetBalancedTreeFactor());
