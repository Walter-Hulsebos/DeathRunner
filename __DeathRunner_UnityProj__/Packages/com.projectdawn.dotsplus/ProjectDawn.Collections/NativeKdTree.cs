...
namespace ProjectDawn.Collections
{
        ...
        where TComparer : unmanaged, IKdTreeComparer<TValue>
    {
        ...
        /// <summary>
        /// Creates and schedules a job that releases all resources (memory and safety handles) of this kd tree.
        /// </summary>
        /// <param name="inputDeps">The dependency for the new job.</param>
        /// <returns>The handle of the new job. The job depends upon `inputDeps` and releases all resources (memory and safety handles) of this kd tree.</returns>
        ...
        private unsafe struct DisposeJob : IJob
        {
            ...
            public void Execute()
            {
                ...
                AtomicSafetyHandle.Release(m_Safety);
#endif
                UnsafeKdTree<TValue, TComparer>.Destroy(Data);
            }
        }
    }
}