...
namespace ProjectDawn.Collections.LowLevel.Unsafe
{
        ...
        where T : unmanaged, ISurfaceArea<T>, IUnion<T>
    {
        ...
        public void Build(in NativeArray<T> array)
        {
            ...
            var branches = new UnsafeLinkedPriorityQueue<Branch, BranchComparer>(array.Length, Allocator.Temp);
        ...
        public Handle Add(T value)
        {
            ...
            // Stage 1: find the best sibling for the new leaf
            // This handle will have the lowest cost to insert new value
            var branches = new UnsafeLinkedPriorityQueue<Branch, BranchComparer>(Length, Allocator.Temp);
            int bestHandle = FindBestHandle(value, ref branches);
            branches.Dispose();
            //int bruteForceBestHandle = FindBestHandleBruteForce(value);
            //Assert.AreEqual(GetCost(value, bestHandle), GetCost(value, bruteForceBestHandle));
        ...
        private void CostRecursive(Node* node, ref float cost)
        {
            ...
            cost += node->Value.SurfaceArea();

            CostRecursive(m_Nodes.Ptr + node->LeftChildHandle, ref cost);
            CostRecursive(m_Nodes.Ptr + node->RightChildHandle, ref cost);
        }
        ...
        private void CountLeafsRecursive(Node* node, ref int count)
        {
            ...
            {
                CountLeafsRecursive(m_Nodes.Ptr + node->LeftChildHandle, ref count);
                CountLeafsRecursive(m_Nodes.Ptr + node->RightChildHandle, ref count);
            }
        }
        ...
        private void GetDepthRecursive(Node* node, int depth, ref int maxDepth)
        {
            ...
            {
                depth++;
                GetDepthRecursive(m_Nodes.Ptr + node->LeftChildHandle, depth, ref maxDepth);
                GetDepthRecursive(m_Nodes.Ptr + node->RightChildHandle, depth, ref maxDepth);
            }
        }
        ...
        public float GetBalancedTreeFactor()
        {
            ...
            int minDepth = GetBalancedDepth(leafCount);

            // Returns depth of unbalanced tree with leafeCount
            int maxDepth = leafCount - 1;
            ...
            float factor = (depth - minDepth) / (maxDepth - minDepth);
