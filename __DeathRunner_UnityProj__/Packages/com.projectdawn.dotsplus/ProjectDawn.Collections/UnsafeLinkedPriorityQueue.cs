...
namespace ProjectDawn.Collections.LowLevel.Unsafe
{
    ...
    /// It is implemented using linked list. Peek = O(1), Enqueue = O(n), Dequeue = O(1).
    /// </summary>
    ...
    public unsafe struct UnsafeLinkedPriorityQueue<TValue, TComparer>
        : IDisposable
        , IEnumerable<TValue>
        ...
        where TComparer : unmanaged, IComparer<TValue>
    {
        ...
        public UnsafeLinkedPriorityQueue(int initialCapacity, AllocatorManager.AllocatorHandle allocator, TComparer comparer = default)
        {
        ...
        /// <param name="comparer">The element comparer to use.</param>
        public static UnsafeLinkedPriorityQueue<TValue, TComparer>* Create(int initialCapacity, AllocatorManager.AllocatorHandle allocator, TComparer comparer = default)
        {
            UnsafeLinkedPriorityQueue<TValue, TComparer>* data = AllocatorManager.Allocate<UnsafeLinkedPriorityQueue<TValue, TComparer>>(allocator);
        ...
        /// Destroys container.
        /// </summary>
        public static void Destroy(UnsafeLinkedPriorityQueue<TValue, TComparer>* data)
        {
